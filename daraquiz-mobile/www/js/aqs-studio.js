...
(function () {
    ...
    async function sendMessage(text, _retryCount) {
        ...
        window.groqFetch({
            ...
            if(window._AQS_ADMIN_MODE){console.error('[dts] AI error:', err);}else{window._aqsAIErrorLog=window._aqsAIErrorLog||[];window._aqsAIErrorLog.unshift({t:new Date().toISOString(),level:'error',msg:'Studio AI error: '+(err&&err.message||err)});};
            isSending = false;
            setSendState(false);
        });
    }
    ...
    function startVoiceListening() {
        /* Guard: never start mic while AI is speaking — prevents echo on device */
    ...
    async function handleVoiceInput(text) {
        ...
        window.groqFetch({
            ...
            speakStudioChunked(spoken, function () {
                ...
                if (voiceActive) {
                    setVoiceState('idle');
                    /* Auto-restart mic after AI finishes — continuous loop, no echo */
                    setTimeout(function () {
        ...
        if (voiceToggle) {
            voiceToggle.addEventListener('click', function () {
                ...
                if (state === 'idle' || state === 'error') {
                    ...
                    stopAiSpeech();
                    setVoiceState('idle');
                    /* Interrupt: mic turns back on immediately so user can keep talking */
