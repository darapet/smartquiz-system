...
async function wikiSearch(q) {
    var r = await fetch(WIKI_API + '?action=query&list=search&srsearch=' + encodeURIComponent(q) + '&srlimit=6&format=json&origin=*', {signal:(function(){var _c=new AbortController();setTimeout(function(){_c.abort();},8000);return _c.signal;})() });
...
async function bookSearch(q) {
    var r = await fetch(BOOKS_API + '?q=' + encodeURIComponent(q) + '&maxResults=6&orderBy=relevance', {signal:(function(){var _c=new AbortController();setTimeout(function(){_c.abort();},8000);return _c.signal;})() });
    if (!r.ok) return [];
    var d = await r.json();
...
async function loadWiki(title) {
    ...
    try {
        var [sumRes, secRes] = await Promise.all([
            fetch('https://en.wikipedia.org/api/rest_v1/page/summary/' + encodeURIComponent(title.replace(/ /g,'_')), {signal:(function(){var _c=new AbortController();setTimeout(function(){_c.abort();},10000);return _c.signal;})() }),
            fetch(WIKI_API + '?action=parse&page=' + encodeURIComponent(title) + '&prop=sections&format=json&origin=*', {signal:(function(){var _c=new AbortController();setTimeout(function(){_c.abort();},10000);return _c.signal;})() })
        ]);
...
async function fetchWikiSection(title, sectionIdx) {
    if (sectionIdx === 0) {
        var r = await fetch('https://en.wikipedia.org/api/rest_v1/page/summary/' + encodeURIComponent(title.replace(/ /g,'_')), {signal:(function(){var _c=new AbortController();setTimeout(function(){_c.abort();},10000);return _c.signal;})() });
        var d = await r.json();
        return d.extract || d.description || '';
    }
    var r2 = await fetch(WIKI_API + '?action=parse&page=' + encodeURIComponent(title) + '&prop=wikitext&section=' + sectionIdx + '&format=json&origin=*', {signal:(function(){var _c=new AbortController();setTimeout(function(){_c.abort();},10000);return _c.signal;})() });
...
async function summonStreamResponse(messages) {
    ...
    var res = await fetch(GROQ_STREAM_URL, {
        ...
        signal:(function(){var _c=new AbortController();setTimeout(function(){_c.abort();},45000);return _c.signal;})() 
    });
