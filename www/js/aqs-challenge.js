/* AQS Challenge Mode v2 — aqs-challenge.js */
  ...
  function startMic(){
      ...
      navigator.mediaDevices.getUserMedia({
          ...
          /* iOS Safari only supports audio/mp4; Android Chrome prefers webm;codecs=opus */
  ...
  /* ── Sound FX ──────────────────────────────────────────────────────── */
  /* Shared AudioContext — iOS limits to 6 simultaneous contexts; reusing one prevents audio death */
  var _sharedBeepCtx=null;
  function _getBeepCtx(){if(!_sharedBeepCtx||_sharedBeepCtx.state==='closed'){_sharedBeepCtx=new(window.AudioContext||window.webkitAudioContext)();}if(_sharedBeepCtx.state==='suspended')_sharedBeepCtx.resume();return _sharedBeepCtx;}
  function beep(f,d,t,v){try{var ctx=_getBeepCtx(),o=ctx.createOscillator(),g=ctx.createGain();o.type=t||'sine';o.frequency.value=f;g.gain.setValueAtTime(v||.4,ctx.currentTime);g.gain.exponentialRampToValueAtTime(.001,ctx.currentTime+d);o.connect(g);g.connect(ctx.destination);o.start();o.stop(ctx.currentTime+d);}catch(e){}}
