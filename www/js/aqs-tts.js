...
(function () {
    ...
    function speakWithBrowser(text, speed, voiceObj) {
        return new Promise(function(resolve, reject) {
            ...
            /* Prime iOS speechSynthesis engine immediately (must happen synchronously on user gesture) */
    ...
    async function generate() {
        ...
        /* Revoke previous blob URL to prevent memory leak (especially on mobile) */
        if (window._aqsTtsLastUrl) { try { URL.revokeObjectURL(window._aqsTtsLastUrl); } catch(e){} }
        var url       = URL.createObjectURL(blob);
        window._aqsTtsLastUrl = url;
