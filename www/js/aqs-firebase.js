...
async function actionSocialLogin(data) {
    ...
    /* Mobile/Capacitor: popups are blocked on iOS/Android — use redirect instead */
    var isCapacitor = !!(window.Capacitor && window.Capacitor.isNativePlatform && window.Capacitor.isNativePlatform());
    var isMobileBrowser = /android|iphone|ipad|ipod/i.test(navigator.userAgent) && !isCapacitor;
    var cred;
    if (isCapacitor || isMobileBrowser) {
        await signInWithRedirect(auth, authProvider);
        return; /* Page will reload; result picked up by getRedirectResult() below */
    } else {
