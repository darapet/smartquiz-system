    ...
    function playBootChime(primary, musicUrl) {
        try {
            ...
            /* Fallback: synthesised chime (C5 → E5 → G5)
               iOS/Android require a user gesture before AudioContext can play audio.
               We attempt creation here; if the context is suspended (mobile), we
               resume it first via a touch listener so the chime fires on next tap. */
            var AudioCtx = window.AudioContext || window.webkitAudioContext;
            if (!AudioCtx) return;
            var actx = new AudioCtx();
            if (actx.state === 'suspended') {
                var _resume = function() { actx.resume().catch(function(){}); document.removeEventListener('touchstart', _resume); document.removeEventListener('click', _resume); };
