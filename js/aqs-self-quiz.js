(function(){
  ...
  (function(){
    ...
    if(!shared)return;
     window.groqFetch=function(body){
       var request=Object.assign({model:window._AQS_GROQ_MODEL||'openai/gpt-oss-20b'},body||{});
       if(typeof window.quizGroqFetch==='function'){
         return window.quizGroqFetch(request).then(function(res){
           if(res&&res.ok)return res;
           return shared(request);
         }).catch(function(){return shared(request);});
      }
       return shared(request);
    };
  }());
  ...
  async function generateSection(s,card){if(s.source==='topic'&&!s.topic.trim()){toast('Add a topic before generating this section.');return;}if(s.source==='doc'&&!s.doc){toast('Choose a readable document before generating this section.');return;}var btn=$('[data-generate]',card),status=$('.section-status',card);btn.disabled=true;btn.textContent='Generating…';status.style.display='block';status.className='section-status';status.textContent='AI is building section '+(sections.indexOf(s)+1)+'…';try{var res=await requestQuizAI({messages:[{role:'system',content:'You are a precise quiz generator. Output only valid JSON.'},{role:'user',content:promptFor(s)}],max_tokens:6144,temperature:.15,response_format:{type:'json_object'}});if(!res||!res.ok){var errorBody=await (res&&res.json?res.json():Promise.resolve({}));var errorMessage=errorBody&&errorBody.error&&errorBody.error.message;throw new Error(errorMessage||'The AI service could not generate this section. Please try again.');}var body=await res.json();var raw=body.choices&&body.choices[0]&&body.choices[0].message&&body.choices[0].message.content||'';var qs=parseQuestions(raw);if(!qs.length)throw new Error('AI returned an invalid question set. Please try again.');qs=qs.slice(0,s.count).map(function(q){q.type=q.type||'mcq';q.options=Array.isArray(q.options)?q.options:[];q.correct_answer_index=parseInt(q.correct_answer_index)||0;q.answer=q.answer||q.options[q.correct_answer_index]||'';return q;});s.questions=qs;s.generated=qs.length;generated=sections.reduce(function(a,x){return a.concat(x.questions||[]);},[]);status.className='section-status success';status.textContent='Generated '+qs.length+' of '+s.count+' requested questions. You can use these now or regenerate for another set.';renderSections();}catch(e){status.className='section-status error';status.textContent=e.message||'Generation failed. Try again.';btn.disabled=false;btn.textContent='Generate section';}}
