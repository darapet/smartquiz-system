...
function subscribeFeed() {
  ...
  state.feedUnsub = onSnapshot(query(collection(db, 'studyco_posts'), orderBy('createdAt', 'desc'), limit(30)), (snapshot) => {
    ...
    /* Author names are hydrated after the first cards are visible. */
...
async function bootApp() {
  /* Show the app shell before any profile, preference, or social-list read.
     Each subscription can paint its own section as soon as its data arrives. */
  showApp();
  renderProfile();
