...
async function handleAction(data) {
    ...
    switch(action) {
        ...
        /* ── LIBRARY ADMIN ── */
        case 'aqs_get_library_profiles': return await actionGetLibraryProfiles();
...
/* ============================================================
   LIBRARY ADMIN — list all library hosts (library_profiles)
   ============================================================ */
async function actionGetLibraryProfiles() {
    try {
        var snap = await getDocs(collection(db, 'library_profiles'));
        var profiles = [];
        snap.forEach(function(d) {
            profiles.push(Object.assign({ id: d.id }, d.data()));
        });
        /* Sort by updatedAt desc (most recent first) */
        profiles.sort(function(a, b) {
            var ta = a.updatedAt ? (a.updatedAt.seconds || a.updatedAt._seconds || 0) : 0;
            var tb = b.updatedAt ? (b.updatedAt.seconds || b.updatedAt._seconds || 0) : 0;
            return tb - ta;
        });
        return profiles;
    } catch(e) {
        console.error('[AQS] actionGetLibraryProfiles:', e);
        return [];
    }
}
